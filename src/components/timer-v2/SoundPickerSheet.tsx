...
export function SoundPickerSheet({ visible, title, cue, masterVolume, onChange, onClose }: Props) {
  ...
  const preview = async (sound: SoundRef) => {
    ...
    setPreviewing(didStart ? key : null)
  }
  ...
  const pickAndroid = async (ringtoneType: 'alarm' | 'notification') => {
    ...
    if (result) onChange({ sound: { kind: 'android', ringtoneType, ...result } })
  }
  const pickDocument = async () => {
    const result = await ChandasTimerService.pickAudioDocument()
    if (result) onChange({ sound: { kind: 'document', ...result } })
  }
    ...
    <BottomSheet visible={visible} eyebrow="SOUND & LEVEL" title={title} onClose={close}>
      <View style={[styles.tabs, { borderColor: tokens.border }]}> 
        {([['built-in', 'Built-in'], ['android', 'Android'], ['device', 'Device']] as const).map(([value, label]) => (
          <Pressable key={value} onPress={() => setTab(value)} accessibilityRole="tab" accessibilityState={{ selected: tab === value }} style={[styles.tab, tab === value && { backgroundColor: tokens.accent }]}>
      ...
      {tab === 'built-in' ? <View style={styles.list}>{BUILT_IN_SOUNDS.map(sound => {
        ...
        return <View key={sound.id} style={[styles.option, { borderColor: selected ? tokens.accent : tokens.border, backgroundColor: selected ? tokens.accentGlow : 'transparent' }]}>
          <Pressable style={styles.optionCopy} onPress={() => onChange({ sound: ref })} accessibilityRole="radio" accessibilityState={{ checked: selected }}>
      ...
      {tab === 'android' ? <View style={styles.sourcePanel}>
        ...
        <Pressable disabled={!isNativeServiceAvailable} onPress={() => void pickAndroid('alarm')} style={[styles.sourceButton, { borderColor: tokens.border }]}>
        ...
        <Pressable disabled={!isNativeServiceAvailable} onPress={() => void pickAndroid('notification')} style={[styles.sourceButton, { borderColor: tokens.border }]}>
      ...
      {tab === 'device' ? <View style={styles.sourcePanel}>
        ...
        <Pressable disabled={!isNativeServiceAvailable} onPress={() => void pickDocument()} style={[styles.sourceButton, { borderColor: tokens.border }]}>
      ...
      <View style={[styles.selected, { backgroundColor: tokens.surfaceHi }]}>
        <View style={styles.optionCopy}><Text style={[styles.label, { color: tokens.textMuted }]}>SELECTED</Text><Text numberOfLines={1} style={[styles.optionTitle, { color: tokens.text }]}>{soundTitle(cue.sound)}</Text></View>
      ...
      <Slider minimumValue={0} maximumValue={1} step={0.05} value={cue.volume} onValueChange={volume => onChange({ volume })} minimumTrackTintColor={tokens.accent} maximumTrackTintColor={tokens.surfaceHi} thumbTintColor={tokens.accent} accessibilityLabel={`${title} volume`} />
