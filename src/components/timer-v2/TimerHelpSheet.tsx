...
export function TimerHelpSheet({ visible, onClose, onOpenFocusSettings }: { visible: boolean; onClose: () => void; onOpenFocusSettings?: () => void }) {
  ...
  return <BottomSheet visible={visible} title="Timer help" onClose={onClose}>
    ...
    {item(<BellIcon on color={tokens.accent} />, 'Advanced', 'At the bottom of setup, pull upward past Show advanced—or tap it—to reveal schedules, configurations, appearance, alarms, second precision, haptics, Focus and system integrations. Hide it again for the simplest timer view.')}
    {item(<BellIcon on color={tokens.accent} />, 'Shortcuts', 'Use the scrolling quick choices for common values. Tap cue positions to select them, swipe list items left to reveal Delete, and hold dotted handles to reorder Sequence steps.')}
  </BottomSheet>
}
