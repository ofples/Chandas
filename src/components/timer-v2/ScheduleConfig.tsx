...
import { windowsOverlap } from '../../lib/activeHours'
  ...
  return <View style={styles.section}>
    ...
    {value.enabled ? <Animated.View entering={FadeInDown.duration(reducedMotion ? 80 : 180)} exiting={FadeOut.duration(reducedMotion ? 70 : 120)} layout={reducedMotion ? undefined : LinearTransition.duration(150)} style={styles.windowList}>
      ...
      {value.weeklyWindows.map((window, index) => {
        ...
        const overlaps = value.weeklyWindows.some(other => other.id !== window.id && other.enabled && window.enabled && windowsOverlap(window, other))
        return <Animated.View key={window.id} layout={reducedMotion ? undefined : LinearTransition.duration(150)} style={[styles.window, { borderColor: editing ? tokens.accent : tokens.border, backgroundColor: tokens.surface, opacity: window.enabled ? 1 : 0.55 }]}>
          <View style={styles.windowHead}><Pressable onPress={() => setEditingId(editing ? null : window.id)} style={styles.flex} accessibilityRole="button" accessibilityLabel={`Edit ${windowSummary(window)}`}><Text style={[styles.rowTitle, { color: tokens.text }]}>{formatRange(window)}</Text><Text style={[styles.helper, { color: tokens.textMuted }]}>{daySummary(window.days)}{overlaps ? ' · overlaps another range' : ''}</Text></Pressable><Toggle value={window.enabled} onChange={enabled => patchWindow(window.id, { enabled })} accessibilityLabel={`Enable time range ${index + 1}`} /></View>
...
const styles = StyleSheet.create({
  ...
  windowList: { gap: 10 }, window: { borderWidth: 1.5, borderRadius: 14, padding: 12, gap: 12 }, windowHead: { minHeight: 40, flexDirection: 'row', alignItems: 'center', gap: 12 }, editor: { borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: 'rgba(127,127,127,0.25)', paddingTop: 12, gap: 12 },
  ...
  empty: { borderWidth: 1.5, borderStyle: 'dashed', borderRadius: 13, padding: 14, gap: 4 }, add: { minHeight: 47, borderWidth: 1.5, borderStyle: 'dashed', borderRadius: 13, alignItems: 'center', justifyContent: 'center' }, addText: { fontSize: 12, fontWeight: '700' },
})
