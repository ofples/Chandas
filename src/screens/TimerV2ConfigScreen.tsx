...
function SequenceStepEditorSheet({ state, stepId, onChange, onEditCue, onClose }: { state: TimerV2State; stepId: string; onChange: (state: TimerV2State) => void; onEditCue: () => void; onClose: () => void }) {
  ...
  return <BottomSheet visible eyebrow={`STEP ${index + 1} OF ${program.steps.length}`} title={<EditableTitle value={step.label} onCommit={label => onChange(patchSequenceStep(state, step.id, { label }))} accessibilityLabel={`Step ${index + 1} name`} large />} accessibilityTitle={step.label} onClose={onClose}>
    ...
    <VolumeControl label="Volume" detail="This step" value={step.volume} onChange={volume => onChange(patchSequenceStep(state, step.id, { volume }))} />
...
function TrackEditorSheet({ state, trackId, onChange, onEditCue, onClose }: { state: TimerV2State; trackId: string; onChange: (state: TimerV2State) => void; onEditCue: () => void; onClose: () => void }) {
  ...
  return <BottomSheet visible eyebrow={`SUB-BELL ${index + 1}${track.enabled ? '' : ' · OFF'}`} title={<EditableTitle value={track.label} onCommit={label => onChange(patchPatternTrack(state, track.id, { label }))} accessibilityLabel={`Sub-bell ${index + 1} name`} large />} accessibilityTitle={track.label} onClose={onClose}>
    ...
    <VolumeControl label="Volume" detail="This sub-bell" value={track.volume} onChange={volume => onChange(patchPatternTrack(state, track.id, { volume }))} />
