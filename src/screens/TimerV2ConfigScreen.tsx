...
import { Modal, PanResponder, Platform, Pressable, ScrollView, StyleSheet, Text, TextInput, Vibration, View } from 'react-native'
...
function TrackEditor({ state, onChange, track, index, total, mainMinutes, onEditCue }: { state: TimerV2State; onChange: (state: TimerV2State) => void; track: PatternTrack; index: number; total: number; mainMinutes: number; onEditCue: (cue: EditingCue) => void }) {
  ...
  return <View style={[styles.track, { borderColor: track.enabled ? tokens.border : tokens.surfaceHi, opacity: track.enabled ? 1 : 0.52 }]}>
    <View style={styles.line}>
      <View style={styles.dragLine}><DragHandle onMoveUp={index > 0 ? () => onChange(reorderPatternTracks(state, index, index - 1)) : undefined} onMoveDown={index < total - 1 ? () => onChange(reorderPatternTracks(state, index, index + 1)) : undefined} /><Text style={[styles.rowTitle, { color: tokens.text }]}>{soundTitle(track.sound)}</Text></View>
...
/** A compact one-slot drag gesture with haptic feedback; arrow buttons remain as an accessible alternative. */
function DragHandle({ onMoveUp, onMoveDown }: { onMoveUp?: () => void; onMoveDown?: () => void }) {
  const { tokens } = useTheme()
  const handlers = useMemo(() => PanResponder.create({
    onStartShouldSetPanResponder: () => Boolean(onMoveUp || onMoveDown),
    onMoveShouldSetPanResponder: (_, gesture) => Math.abs(gesture.dy) > 4,
    onPanResponderGrant: () => Vibration.vibrate(8),
    onPanResponderRelease: (_, gesture) => {
      if (gesture.dy < -24) onMoveUp?.()
      if (gesture.dy > 24) onMoveDown?.()
    },
  }), [onMoveDown, onMoveUp])
  return <View {...handlers.panHandlers} style={styles.dragHandle} accessibilityRole="adjustable" accessibilityLabel="Drag to reorder"><Text style={[styles.drag, { color: tokens.textMuted }]}>⠿</Text></View>
}
