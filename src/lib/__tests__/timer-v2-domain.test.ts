...
describe('timer v2 validation and presets', () => {
  ...
  it('migrates legacy timing, disabled sub-bells, settings, and snap without deletion semantics', () => {
    ...
    expect(migrated.settings).toMatchObject({ masterVolume: 0.35, alarmSound: { kind: 'builtin', id: 'alarm-tone' }, alarmVolume: 1, availability: { enabled: true, weeklyWindows: [{ days: 0b0101010 }] }, focusAutomationEnabled: true })
