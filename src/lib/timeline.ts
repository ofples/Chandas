...
function completionCandidate(program: TimerProgram): TimelineCueCandidate {
  const cue = program.endCue ?? (program.mode === 'pattern' ? program.mainCue : program.steps.at(-1)!)
  return { cueId: program.endCue ? 'end' : 'completion', kind: 'run-complete', sound: cue.sound, volume: cue.volume }
}
...
export function nextProgramEvent(program: TimerProgram, anchor: number, now = Date.now(), startedAt = anchor, terminalAt = runEndAt(program, anchor, startedAt)): ScheduledProgramEvent | null {
  ...
  if (next.at === endAt) {
    if (!program.endCue) return { ...next, completesRun: true }
    const winner = completionCandidate(program)
    // Preserve the natural boundary identity so cycle-mute and alarm-once
    // semantics still recognize the final main/cycle boundary. Only its cue
    // is replaced by the explicitly configured ending gong.
